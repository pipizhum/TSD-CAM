...
parser.add_argument("--base_dir", default='./cos_sim', type=str, help="train/val/test list file")
...
def _validate(model=None, data_loader=None, args=None):
    ...
    with torch.no_grad(), torch.cuda.device(0):
        ...
        for idx, data in tqdm(enumerate(data_loader), total=len(data_loader), ncols=100, ascii=" >="):
            ...
            patch_token, attn, cls_token = model(inputs)
            ###
            ...
            patch_token, attn, cls_token = model(F.interpolate(local_crops, size=(448, 448), mode="bilinear", align_corners=False))
