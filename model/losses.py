...
def get_masked_ptc_loss(inputs, mask):
    b, c, h, w = inputs.shape   # (2, 768, 28, 28)
    
    inputs = inputs.reshape(b, c, h*w)  # (2, 768, 784)

    def cos_sim(x):
        x = F.normalize(x, p=2, dim=1, eps=1e-8)    # (2, 768, 784)
        cos_sim = torch.matmul(x.transpose(1,2), x)   # (2, 784, 784)
        return torch.abs(cos_sim)

    inputs_cos = cos_sim(inputs)    # (2, 784, 784)
...
class CTCLoss_neg(nn.Module):
    ...
    def forward(self, student_output, teacher_output, flags):
        ...
        b = flags.shape[0]  # batch_size

        student_out = student_output.reshape(self.ncrops, b, -1).permute(1, 0, 2)   # (b, 10, 1024)
        teacher_out = teacher_output.reshape(2, b, -1).permute(1, 0, 2)  # (b, 2, 1024)

        logits = torch.matmul(teacher_out, student_out.permute(0, 2, 1))  # (b, 2, 10) 可以理解为学生和教师之间的相似性分数
        logits = torch.exp(logits / self.temp)  # (b, 2, 10)

        total_loss = 0
        for i in range(b):  # 遍历所有的batch
            neg_logits = logits[i, :, flags[i] == 0]  # 从logits中提取出对应标志位为0的负样本的分数(2, neg_classes),其中neg_classes是负样本的数量。
            pos_inds = torch.nonzero(flags[i])[:, 0]  # 正样本的索引位置
            loss = 0

            for j in pos_inds:
                pos_logit = logits[i, :, j]  # 取出正样本分数
                loss += -torch.log((pos_logit) / (pos_logit + neg_logits.sum(dim=1) + 1e-4))
            else:
                loss += -torch.log((1) / (1 + neg_logits.sum(dim=1) + 1e-4))
                
            total_loss += loss.sum() / 2 / (pos_inds.shape[0] + 1e-4)   # 计算总损失

        total_loss = total_loss / b  # 总损失除以batch_size
