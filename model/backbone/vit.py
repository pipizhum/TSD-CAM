...
class VisionTransformer(nn.Module):
    ...
    def prepare_tokens(self, x):
        ...
        x = self.patch_embed(x)  # patch linear embedding   # (2, 784, 768)
        ...
        pos_embed = torch.cat((self.pos_embed[:, :1, :], patch_pos_embed), dim=1)   # (1, 785, 768)

        # add the [CLS] token to the embed patch tokens
        cls_tokens = self.cls_token.expand(B, -1, -1)   # (b/2b, 1, 768)
        x = torch.cat((cls_tokens, x), dim=1)   # (b/2b, 785, 768)

        # add positional encoding to each token
        x = x + pos_embed   # pos_embed -> (1, 785, 768)
    ...
    def forward_features(self, x):
        ...
        x = self.norm(x)    # (b/2b, 785, 768)
        embeds[-1] = x
        return x[:, 0], x[:, 1:], embeds[self.aux_layer][:, 1:]  # (b/2b, 1, 768) (b/2b, 784, 768) (b/2b, 784, 768)
...
def vit_large_patch16_224(pretrained=False, **kwargs):
    ...
    if pretrained:
        load_pretrained(model, pretrained_cfg=default_cfgs, num_classes=model.num_classes, in_chans=kwargs.get('in_chans', 3))
    return model
