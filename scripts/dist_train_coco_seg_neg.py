...
def train(args=None):
    ...
    for n_iter in range(args.max_iters):
        ...
        if n_iter <= 12000:
            refined_pseudo_label = refine_cams_with_bkg_v2(varm, inputs_denorm, cams=valid_cam_aux, cls_labels=cls_label, high_thre=args.high_thre, low_thre=args.low_thre, ignore_index=args.ignore_index, img_box=img_box, )
        else:
        ...
        if n_iter <= 8000:
            loss = 1.0 * cls_loss + 1.0 * cls_loss_aux + 0.0 * ptc_loss + 0.0 * seg_loss + 0.0 * reg_loss
        elif n_iter <= 12000:
            loss = 1.0 * cls_loss + 1.0 * cls_loss_aux + 0.0 * ptc_loss + 0.1 * seg_loss + args.w_reg * reg_loss
        else:
            loss = 1.0 * cls_loss + 1.0 * cls_loss_aux + 0.2 * ptc_loss + 0.1 * seg_loss + args.w_reg * reg_loss + 0.1 * loss_corr + 0.1 * loss_er
