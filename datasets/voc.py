...
class VOC12ClsDataset(VOC12Dataset):
    def __init__(self,
        ...
        self.global_view2 = T.Compose([
            # RandomResizedCrop随机裁剪和调整大小,self.crop_size:裁剪后的目标尺寸,interpolation=Image.BICUBIC：插值方法，用于调整大小时的像素插值
    ...
    def __transforms(self, image):
        ...
        if self.aug:
            ...
            local_image = self.local_view(Image.fromarray(image))   # 数据增强处理 (3, 448, 448)
            image = self.global_view1(Image.fromarray(image))
        
        image = self.normalize(image)   # (3, 448, 448)
        
        return image, local_image, img_box  # (3, 448, 448) (3, 448, 448)
    ...
    def __getitem__(self, idx):
        ...
        pil_image = Image.fromarray(image)  # 将Numpy或ndarray转换为PIL对象，便于使用PIL库中提供的各种方法
        ...
        if self.aug:
            ...
            crops = []  # 存储1.进行transforms后的图像;2.原始图像;3.image图像经过变化后的图像,后续会进行local_crop局部裁剪
            crops.append(image)
            crops.append(self.global_view2(pil_image))  # 原始图像
