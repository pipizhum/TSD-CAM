...
def _img_rescaling(image, label=None, scale=None):
    ...
    h, w, _ = image.shape   # 图像原始大小，如(3, 375, 500)
...
def random_crop(image, label=None, crop_size=None, mean_rgb=[0,0,0], ignore_index=255):
    ...
    h, w, _ = image.shape   # 图像原始大小

    H = max(crop_size, h)   # 将图像大小与裁剪大小比较，取较大者
    W = max(crop_size, w)   # 将图像大小与裁剪大小比较，取较大者

    pad_image = np.zeros((H, W, 3), dtype=np.uint8)   # (H, W, 3)定义与原图像大小相同的空数组
    
    pad_image[:, :, 0] = mean_rgb[0]
    pad_image[:, :, 1] = mean_rgb[1]
    pad_image[:, :, 2] = mean_rgb[2]
    
    H_pad = int(np.random.randint(H - h + 1))   # 当crop_size大于原始图像时，从他们的差中随机取一个作为H_pad
    W_pad = int(np.random.randint(W - w + 1))   # 当crop_size大于原始图像时，从他们的差中随机取一个作为W_pad
    # 若图像大小小于裁剪大小，则填充图像
    ...
    crop_image = pad_image[H_start:H_end, W_start:W_end, :]   # 裁剪图像(448, 448, 3)
